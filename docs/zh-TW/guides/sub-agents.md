...
列表/元組的形狀是 `[soft, hard]`。軟限制會在子代理下一輪 LLM 呼叫前注入提醒；硬限制會阻止繼續派發工具/子代理，讓專家用文字收尾，而不是繼續消耗執行預算。
...
| 包 | 展開為 | 何時使用 |
|---|---|---|
| `budget` | `budget.ticker`, `budget.alarm`, `budget.gate` | 需要 turn/tool/walltime 預算計數與執行。 |
| `auto-compact` | `compact.auto` | 已設定 `compact`，並希望在 LLM 輪次後自動檢查壓縮。 |
| `default-runtime` | 上述全部 | 常規子代理執行期安全預設值。 |
...
同名的使用者宣告外掛會覆蓋預設外掛，因此你可以替換某一個預算外掛，同時保留外掛包裡的其它外掛。

## 父代理預算與子代理預算

子代理上的 `turn_budget` 和 `tool_call_budget` 是該子代理執行的獨立多軸預算。
...
新設定優先使用明確的子代理 `turn_budget` / `tool_call_budget` 加 `default_plugins: ["default-runtime"]`。只有在你想讓父代理和子代理共享同一個全域上限時，才使用 `max_iterations`。
...
`compact.auto` 外掛會在 LLM 輪次後檢查用量，並在達到閾值時觸發壓縮。沒有 `compact.auto`（或 `default-runtime`）時，單獨的 `compact:` 只會設定管理器，不會自動觸發。
...
3. 至少設定 `turn_budget: [40, 60]` 和 `tool_call_budget: [75, 100]`。
