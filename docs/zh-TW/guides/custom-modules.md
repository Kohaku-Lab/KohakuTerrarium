...
每個模組就是一支 Python 檔 (放哪都可以 — 通常放在生物資料夾裡、或某個套件裡)。Config 用 `module: ./path/to/file.py` + `class_name: YourClass` 指過去。
...
- **工具沒出現在 prompt 裡。** 跑 `kt info path/to/creature`。八成是被默默拒絕了 — 確認 `class_name` 有對上。
