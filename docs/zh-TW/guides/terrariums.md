---
title: 生態瓶
summary: 用頻道、輸出接線、特權節點、熱插拔、觀察模式做橫向多代理。
tags:
  - guides
  - terrarium
  - multi-agent
...
- 每隻生物一條以它名字命名的頻道，讓別人可以透過 `send_channel` DM 它。
- 設了 `root` 時，多一條 `report_to_root` 頻道，並把圖中其他每隻生物都接線為可送往該頻道、只讓 root 監聽。
...
Recipe 裡的 `root:` 關鍵字會把圖內的一隻生物提升為[特權節點](../concepts/glossary.md#privileged-node--特權節點)。它從圖內管理團隊，不是從外面：

- 自動 listen 它所在圖中的每一條頻道。
- 收 `report_to_root`，其他每隻生物都被接線為可送往該頻道。
- 拿到[群組工具](../concepts/glossary.md#group-tools--群組工具) (`group_add_node`、`group_remove_node`、`group_start_node`、`group_stop_node`、`group_channel`、`group_wire`、`group_status`)。
- 自動收到一段產生的「圖感知」prompt，列出當前的生物、頻道、listen / send 接線、輸出接線邊；該段會在拓樸變化時刷新。
