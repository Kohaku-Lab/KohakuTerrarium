...
生態瓶是一層**純接線層**：它擁有頻道，並管理生物生命週期。它本身沒有自己的 LLM。智慧保留在每一隻生物內部。完整契約請參見
...
- 頻道只在 `channels:` 中宣告一次。`queue` 會把每則訊息傳給一個 consumer；`broadcast` 則會傳給所有 listener。
...
在 terrarium yaml 旁邊建立 `prompts/root.md` —— 它只需要承載委派風格即可；框架會自動產生拓樸感知區塊，列出團隊中的生物與頻道，並強制注入管理工具組（`terrarium_send`、
`creature_status`、`terrarium_history`、……）。

TUI 會把 root 掛在主分頁上；你和 root 對話，root 再和團隊對話。更多內容請見 [root agent 概念](../concepts/multi-agent/root-agent.md)。

## 你學到了什麼

- 生態瓶就是接線。它不增加任何智慧。
- 生物保持獨立；terrarium 只告訴它們誰能聽到什麼、誰能傳到哪裡，以及它們在回合結束時的輸出會流向何處。
- 兩種協作機制可以自由組合：
...
- [Terrarium 概念](../concepts/multi-agent/terrarium.md) —— 它的契約與邊界。
- [Root agent 概念](../concepts/multi-agent/root-agent.md) —— 面向使用者的生物。
- [Terrariums 指南](../guides/terrariums.md) —— 實用導向的操作參考。
- [Channel 概念](../concepts/modules/channel.md) —— queue 與
  broadcast 的差異、observers，以及頻道如何跨越模組邊界。
