...
`terrarium/runtime.py:TerrariumRuntime.start` (第 85-180 行) 會先把共用頻道建好、每隻生物都有一條 direct queue、有 root 時加上 `report_to_root`、透過 `terrarium/factory.py:build_creature` 組出每隻生物、啟動生物、最後組出 root (但不啟動)，接著啟動終止條件檢查器。

`build_creature` 透過 `@pkg/...` 或路徑載入 base config、建 `Agent(session=Session(creature_name), environment=shared_env, …)`、為每個 listen channel 註冊一個 `ChannelTrigger`，並把頻道拓樸提示串接到 system prompt。生物永遠不會知道自己在生態瓶裡，除非透過頻道或 (選擇性的) 拓樸提示才能察覺。

Root 代理會把 `TerrariumToolManager` 綁到它的 environment，這樣它就能用 `terrarium_*` 與 `creature_*` 工具。Root 永遠在外面，不是生物的同儕。

`terrarium/hotplug.py:HotPlugMixin` 提供執行期的 `add_creature`、`remove_creature`、`add_channel`、`remove_channel`。`terrarium/observer.py:ChannelObserver` 在頻道 send 上掛非破壞性的 callback，讓 dashboard 可以觀察 queue 頻道而不消掉訊息。
