---
title: 边界
summary: Creature 抽象是预设值，不是紧身衣。这一页讲框架会在哪里弯曲自己的抽象，以及什么时候它根本不适合。
tags:
  - concepts
  - philosophy
---
...
这不是抽象泄漏，而是设计重点。**例子：channels**。 channel 并不在聊天机器人 → Agent 的那条推导路径里。它是为了多 Agent 系统引入的通讯基底，而最简单的实现就是「某个 tool 写入消息；某个 trigger 在消息到达时触发」。这等于为了一个概念，把两个模块混在一起用。但这也是最自然的做法；如果硬要假装不是这样，只会多发明一个没有实质收益的新 primitive。**例子：面向用户的特权节点**。`root:` 节点本质上是“一个带有特定工具集和特定监听接线方式的生物 (creature)”。结构上它和其他生物没有差别；要紧的是特权标记，加上引擎套用的标准 report-to-root / listen-all 接线。我们把它点名成独立角色，是因为这个区分有用，不是因为框架强制如此。
