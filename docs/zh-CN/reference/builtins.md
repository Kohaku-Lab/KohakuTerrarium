...
`type: trigger` 来选择启用。工具描述会带上 `**Trigger**: ` 前缀，
让 LLM 知道调用它会安装一个长期存在的副作用。这三个工具都会立即返回已安装的 trigger ID；trigger 本身则在后台执行。
...
| `/edit <message_index> <new content>` | （无） | 编辑过去的 user message，并从该点作为新 branch 重跑。 |
| `/branch [<turn> <branch_id>\|latest]` | `/br` | 列出或切换 regen/edit alternatives 的 live branch。 |
| `/fork [event_id] [--name name]` | （无） | 将当前 session 复制成新的 `.kohakutr` 文件，用于探索替代路线。 |
...
内置 preset 合并时也会吃进已安装套件贡献的 `llm_presets`；见 [configuration.md 的套件 manifest 一节](configuration.md#套件-manifest-kohakuyaml)。

## Variation groups

Variation group 让一个 preset 可以暴露多个旋钮而不必重复条目。在 `llm:` / `--llm` 里用 `preset@group=option` 简写选择，或通过 `controller.variation_selections`；见 [配置参考的 Variation 选择器一节](configuration.md#variation-选择器)。
...
工厂：`Pure`、`Sequence`、`Product`、`Fallback`、`Retry`、`Router`、`Iterator`。包装辅助：`agent(config_path)` 创建持久 Agent，`factory(config)` 创建每次调用临时新建的 Agent。
...
套件的 `kohaku.yaml` 可以贡献 `creatures`、`terrariums`、`tools`、`plugins`、`llm_presets` 与 `python_dependencies`。`kt extension list` 会把它们列出来。Python 模组以 `module:class` 参照解析；config 以 `@pkg/path` 解析。见 [configuration.md 的套件 manifest 一节](configuration.md#套件-manifest-kohakuyaml)。
