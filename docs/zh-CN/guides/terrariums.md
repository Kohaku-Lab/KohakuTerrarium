---
title: 生态瓶 （Terrarium）
summary: 用频道、输出接线、特权节点、热插拔、观察模式做横向多代理协作。
tags:
 - guides
 - terrarium
 - multi-agent
...
- 每只 Creature 一条以它名字命名的频道，让别人可以透过 `send_channel` DM 它。
- 设了 `root` 时，多一条 `report_to_root` 频道，并把图中其他每只 Creature 都接线为可送往该频道、只让 root 监听。
...
Recipe 里的 `root:` 关键字会把图内的一只 Creature 提升为[特权节点](../concepts/glossary.md#privileged-node--特权节点)。它从图内管理团队，不是从外面：

- 自动 listen 它所在图中的每一条频道。
- 收 `report_to_root`，其他每只 Creature 都被接线为可送往该频道。
- 拿到[组工具](../concepts/glossary.md#group-tools--组工具) (`group_add_node`、`group_remove_node`、`group_start_node`、`group_stop_node`、`group_channel`、`group_wire`、`group_status`)。
- 自动收到一段产生的「图感知」prompt，列出当前的 Creature、频道、listen / send 接线、输出接线边；该段会在拓扑变化时刷新。
- Terrarium 跑 TUI/CLI 时，它就是面向用户的介面。
