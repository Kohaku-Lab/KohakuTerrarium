<template>
  <div class="h-full flex flex-col bg-warm-50 dark:bg-warm-900 overflow-hidden">
    <div class="flex items-center gap-2 px-3 py-2 border-b border-warm-200 dark:border-warm-700 shrink-0">
      ...
      <span v-if="creatures.length" class="text-[10px] font-mono px-1.5 py-0.5 rounded bg-warm-100 dark:bg-warm-800 text-warm-400">{{ creatures.length }}</span>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-2 text-xs">
      ...
      <template v-else>
        <div class="text-warm-400 py-6 text-center text-[11px]">{{ t("creatures.notTerrarium") }}</div>
      </template>
    </div>
  </div>
</template>
