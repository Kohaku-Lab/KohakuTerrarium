      ...
      <div class="rounded-lg border border-warm-200 dark:border-warm-700 p-3">
        ...
        <div class="flex flex-col gap-1.5">
          <div class="flex items-center gap-2">
            ...
            <span class="text-warm-600 dark:text-warm-400">
              <span :title="chat.sessionInfo.configRef || instance?.config_ref || ''">{{ chat.sessionInfo.configName || instance?.creature_config_name || instance?.creatures?.[0]?.config_name || chat.sessionInfo.agentName || instance?.config_name || instance?.creatures?.[0]?.name || "--" }}</span>
            </span>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-warm-400 w-16">{{ t("common.model") }}</span>
