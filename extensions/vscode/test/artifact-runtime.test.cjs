...
test('an owned page response admits its embedded refs after the ownership fence', async () => {
  const { client, host } = harness()
  await host.handle({ type: 'session.select', requestId: 1, session: 'graph-live', creatureId: 'creature-beta' })
  client.historyPage = async () => ({ events: [{ image_url: { url: IMG } }] })
  await host.handle({
...
test('an owned detail response admits its embedded refs after the ownership fence', async () => {
  const { client, host } = harness()
  await host.handle({ type: 'session.select', requestId: 1, session: 'graph-live', creatureId: 'creature-beta' })
  client.historyDetail = async () => ({ record: { image_url: { url: IMG } } })
  await host.handle({
    type: 'http.historyDetail',
    requestId: 2,
    session: 'graph-live',
    creature: 'beta',
    params: { stream: 'events', ref: 'opaque-token', history_id: 'history' },
  })
  assert.equal(host.artifacts.allowed(IMG), true)
})

test('a stale page response is rejected and never widens the readable set', async () => {
  const gate = deferred()
  const { client, host } = harness()
  await host.handle({ type: 'session.select', requestId: 1, session: 'graph-live', creatureId: 'creature-beta' })
  client.historyPage = () => gate.promise
  ...
  await Promise.resolve()
  // An explicit selection intent supersedes the in-flight page even though the pointer is unchanged.
  await host.handle({ type: 'session.reconcile', requestId: 3 })
  gate.resolve({ events: [{ image_url: { url: IMG } }] })
  await assert.rejects(() => page, /ownership changed/)
  assert.equal(host.artifacts.allowed(IMG), false)
})

test('a stale detail response is rejected and never widens the readable set', async () => {
  const gate = deferred()
  const { client, host } = harness()
  await host.handle({ type: 'session.select', requestId: 1, session: 'graph-live', creatureId: 'creature-beta' })
  client.historyDetail = () => gate.promise
  const detail = host.handle({
    type: 'http.historyDetail',
    requestId: 2,
    session: 'graph-live',
    creature: 'beta',
  ...
  await Promise.resolve()
  await host.handle({ type: 'session.reconcile', requestId: 3 })
  gate.resolve({ record: { image_url: { url: IMG } } })
